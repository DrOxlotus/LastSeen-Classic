...
addonTbl.AddQuest = function(questID, currentDate)
	...
	if LastSeenClassicQuestsDB[questID] then
		LastSeenClassicQuestsDB[questID]["lastCompleted"] = currentDate;
	else
		LastSeenClassicQuestsDB[questID] = {questTitle = questTitle, lastCompleted = currentDate, questLink = questLink};
	end
end
